...
void  my_read(struct file *f,char __user *buffer, size_t length, loff_t *off)
{
       ...
       printk(KERN_INFO "Twinkle's char reading...\n");
}

void  my_write(struct file *f,char __user *buffer, size_t length, loff_t *off)
{

       printk(KERN_INFO "Twinkle's char writing...\n");
}
...
{
	printk(KERN_INFO "Twinkle's char registered\n");
        
	if(alloc_chrdev_region(&Major_Number,0,10,"Twinkle's assignment1")<0)
	{
		return -1;
	}
         printk(KERN_INFO "Major=%d\n",MAJOR(Major_Number));
	if ((cls=class_create(THIS_MODULE,"Twinkle's chardrv"))==NULL)
	{
...
static void __exit device_exit(void)
{
	...
	class_destroy(cls);
	unregister_chrdev_region(Major_Number,10);
	printk(KERN_INFO "Twinkle's char unregistered\n");
}
